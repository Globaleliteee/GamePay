...
The GamePay Core daemon can be configured to act as a trusted "border
router", implementing the gamepay wire protocol and relay, making
...
The ZeroMQ feature in GamePay Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building gamepayd:
...
For instance:
    ...
    $ gamepayd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/gamepayd.tx.raw \
...
These options can also be provided in gamepay.conf.
...
From the perspective of gamepayd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into gamepayd directly. Furthermore, no information is
...
using. GamePayd appends an up-counting sequence number to each
